...
- `errorMessage`: `string` - The text that will be shown in the error if the assertion fails.
...
- `operation`: `string` - The name of the operation that should be performed by the garbage
collector.
...
- `level`: `int` - The level of information that should be printed. Defaults to 1.
...
- `stack`: `Variant` - The stack level (int) of the environment to be returned; or the
...
- `lastKey`: `Variant` - The last key that was previously retrieved from a call to next.
...
- `module`: `ModuleScript | string | number` - The `Class.ModuleScript` that will be executed to retrieve the return
...
- `f`: `Variant` - Either a function or a number that specifies the function at that
stack level.
...
- `base`: `int` - The numerical base to convert `arg` into.
...
- `i`: `int` - The index of the first element to unpack.
- `j`: `int` - The index of the last element to unpack.
