...
- `formatString`: `string` - Must be either `"*t"` or `"!*t"`.
- `time`: `int` - The time value to format.
...
to that returned by `Library.os.date()`, it returns the number of seconds
...
- `time`: `table` - A dictionary table describing a specific time, similar to that
